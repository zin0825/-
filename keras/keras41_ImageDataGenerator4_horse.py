...
print(y_train.shape, y_test.shape)   # (821,) (206,)
...
#2. 모델 구성
model = Sequential()
model.add(Conv2D(11, (2,2), input_shape=(100, 100, 3), activation='relu',
                 strides=1, padding='same'))
model.add(MaxPooling2D())
model.add(Conv2D(filters=11, kernel_size=(2,2), activation='relu', 
                 strides=1, padding='same'))
model.add(Conv2D(11, (2,2), strides=1, padding='same'))
model.add(Flatten())
...
filepath = "".join([path, 'k41_04_',date, '_' , filename])    # 문자열을 만드는데 아무것도 없는 공문자를 만들고
...
# r2_score :  0.7281297146184107
# 걸린 시간 :  9.75 초
# 로스 :  [0.34449538588523865, 0.9320388436317444]


# acc :  0.9805825242718447
# r2_score :  0.9219696973788589
# 걸린 시간 :  7.63 초
# 로스 :  [0.08212288469076157, 0.9805825352668762]