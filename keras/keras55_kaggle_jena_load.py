...
# # print(x.shape, y.shape)   # (420407, 13) (144, 13)
...
x = x[:-1]
y = y[1:]
y2 = csv.tail(144)   # tail 뒤에서 144개를 가져옴. 꼬리
y2 = y2['T (degC)']
...
# model.fit(x_train, y_train, epochs=550, batch_size=5500,
#           verbose=1, 
#           validation_split=0.3,
#           callbacks=[es])
...
# model.save(".//_data//_save///keras55//k55_06_.h5")


# end = time.time()

model = load_model(np_path + './/_data//_save///keras55//k55_07_.h5')

#4. 평가, 예측
loss = model.evaluate(x, y, batch_size=32)


y_pred = model.predict(x_predict, batch_size=32)
...
def RMSE(y_test, y_pred):   # 여기 있는 함수 정의라서 호출 될 때까지 실행하진 않고 호출 할 때 실햄함 / pred는 무조건 세트고 위에 있는 pred랑 입력값으로 받음
      return np.sqrt(mean_squared_error(y2, y_pred))   # np.sqrt 루트를 씌우다
rmse = RMSE(y_test, y_pred)   # 내가 사용할 함수 명을 사용/ 여기서 입력을 받음, 호출 및 실행
print("RMSE : ", rmse)
# 매개 변수 (입력 받을 값)
...
submit.to_csv(path + "jena_최진영_07.csv", index=False)

# y_submit = model.predict()

# train_csv['T (degC)'] = y_submit
