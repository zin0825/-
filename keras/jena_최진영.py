...
# 그래서 인덱스를 데이터로 변환해서 온도와 상호작용하게 사용게끔 하나 무조건은 아님. 데이터에 따라 다름
...
x = split_x(x, size)  # 스플릿_x라는 함수에 a와 사이즈를 입렵 / split를 표출해준다  여기부터 슬라이싱을 한다
...
x_pred = np.array(x_pred).reshape(1, 144, 14)   # LSTM에 넣기 위해 reshape
# 밑에 남은 쩌리 144개를 테스트로 만들었다



x = x[:-1, :]   # 전체에서 예측이 불가능한 밑에 안쓰는 144개를 뺀 테스는 뺀 값이 요거
y = y[1:]   # y위 맨 위 0번째를 날리고 나머지 1,2,3,4,,, 등만 들어감
# 1에 144개의 데이터가 들어갔음
...
# model.fit(x_train, y_train, epochs=100, batch_size=2224, 
#           verbose=1, 
...
# model.save("C:\\ai5\\_save\\keras55\\jena_최진영.h5")
model = load_model(path2 + 'jena_최진영.h5')
...
submit.to_csv(path2 + "jena_최진영.csv", index=False)
...
# 로스 :  [1.6750249862670898, 0.061366721987724304]
# 걸린 시간 :  655.76 초
# RMSE :  3.4839821961915107



# 로스 :  [1.0089049339294434, 0.09520773589611053]
# RMSE :  1.1512741355389753
# jena_최진영_01.h5


# 로스 :  [0.5734065771102905, 0.08429450541734695]
# RMSE :  0.707267338073789
# jena_최진영.h5
