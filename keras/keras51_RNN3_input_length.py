...
#2. 모델 구성
model = Sequential()
# model.add(SimpleRNN(60, input_shape=3,1))   # 행무시 열우선, 7을 뺀 나머지가 인풋 쉐이프의 모양 cnn, rnn, dnn 모두 동일
# model.add(SimpleRNN(60, input_length=3, input_dim=1))   # 행무시 열우선, 7을 뺀 나머지가 인풋 쉐이프의 모양 cnn, rnn, dnn 모두 동일
model.add(SimpleRNN(60, input_dim=1, input_length=3))   # 반대로 쓸 수 있지만 가독성 떨어짐. 쓰고시프면 써!!!
